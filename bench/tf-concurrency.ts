...
const CONCURRENCY_SWEEP = [1, 2, 4, 8, 16, 32, 64] // generic; NOT the host-side video number
...
// only flag: --warmup-only (load model + one classify, then exit — smoke-test the env)
...
async function main() {
	...
	type Row = { seq: Timed; conc: Timed; batch: Timed }
	...
	for (const N of CONCURRENCY_SWEEP) {
		...
		console.log(line('concurrent', conc))
		console.log(line('batched', batch))
		console.log('------------|----|---------|-------------|-----------')
	}

	// -------------------------------------------------------------------------
	// Derived ratios: the actual decision signals
	// -------------------------------------------------------------------------
